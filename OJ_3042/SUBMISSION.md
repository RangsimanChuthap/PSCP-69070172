...
```text
Take one singular number as an input.

list every number up from 0 to the input number that is divisible by 10.
```
...
Step 2: perform floor division by 10 on the input number to get highest
number that is divisible by 10.
Step 3: Iterate from the result number to 0, using -10 as step.
```
...
```text
Input where the number is divisible by 10
```
...
```text
Input where the number is NOT divisible by 10
```
...
```text
When only number up to current number that is divisible by 10 is 0
```
